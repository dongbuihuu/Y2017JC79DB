	...
	static void PrintInfo(){
		for(int i = 0; i<liLisStudent.size();i++)
			for(int j = 0;j<liLisStudent.get(i).size();j++){
				System.out.println(String.format("- RollNo: %s", 
									liLisStudent.get(i).get(j).getiRollNo()));
				System.out.println(String.format("- FullName: %s", 
						liLisStudent.get(i).get(j).getsFullName()));
				System.out.println(String.format("- Mark: %s", 
						liLisStudent.get(i).get(j).getdMark()));
			}
	}
}
