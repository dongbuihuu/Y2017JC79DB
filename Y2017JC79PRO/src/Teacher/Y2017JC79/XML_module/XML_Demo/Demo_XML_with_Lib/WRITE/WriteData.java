package Teacher.OOP_CONCEPT.XMLMODULES.XML_EXCERCISES.WRITE;
...
public class WriteData {
	public static void main(String[] args) {
		/*
		 * Nghiệp vụ ghi dữ liệu xuống XML Files - List<T> lisT - List<List<Object>>
		 * lisListObject - String sRootNode - String sElementNode - String sPathFile -
		 * boolean isWriteType
		 */
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				boolean isResult = objController.XMLExecute(objXMLParam, (byte) 2);
				if (isResult) {
					JOptionPane.showMessageDialog(null, "Ghi dữ liệu xuống file thành công!!!");
					int iOpt = JOptionPane.showConfirmDialog(null,
							"Bạn có muốn xem dữ liệu vừa ghi xuống file hay không?", "Thông báo",
