...
public class Views {
	public static void main(String[] args) {
		...
		// 2. Thiết lập đầy đủ thông tin cho đối tượng <đóng gói dữ liệu vào đối
		// tượng>
		objEntity.setdNumA(8.5);
		objEntity.setdNumB(9.5);

		/*
		 * Quy ước với nhau như sau: + bOpt = 0: Cộng + bOpt = 1: Trừ + bOpt =
		 * 2: Nhân + bOpt = 3: Chia
		 */
		...
		if (objEntity.getbOpt() < 0 || objEntity.getbOpt() > 3) {
			...
			if (objEntity.getbOpt() == 3) {
				if (objEntity.getdNumA() == 0 && objEntity.getdNumB() == 0)
					...
					System.out.println("Thương 2 số a và b là: " + objController.Calculators(objEntity));
			} else {
				// 5. Hiển thị kết quả
				System.out.println("Kết quả là: " + objController.Calculators(objEntity));
			}
		}
	}
}
