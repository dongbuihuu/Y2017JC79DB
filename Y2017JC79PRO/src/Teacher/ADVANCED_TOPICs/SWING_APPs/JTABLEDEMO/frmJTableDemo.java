		...
		btnLyThngTin.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (jtbLoadData.getSelectedRow() < 0) {
					JOptionPane.showMessageDialog(getParent(), "Chú cần phải chọn cho anh 1 bản ghi nào đó :D");
				} else {
					// 2. Lấy thông tin của bản ghi nào đó --> Bindding vào đối tượng mà chúng ta
					// có?
					StudentObject objStudent = new StudentObject();
					objStudent.setiRollNo(
							Integer.parseInt(String.valueOf(m_objDTM.getValueAt(jtbLoadData.getSelectedRow(), 0))));
					objStudent.setsFullName(String.valueOf(m_objDTM.getValueAt(jtbLoadData.getSelectedRow(), 1)));
					objStudent.setdMark(
							Double.parseDouble(String.valueOf(m_objDTM.getValueAt(jtbLoadData.getSelectedRow(), 2))));

					// 3. Khai báo và khởi tạo 1 đối tượng của form khác --> gửi cho nó đối tượng đã
					// chứa
					// đầy đủ thông tin
