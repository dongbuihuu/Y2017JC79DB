...
public class ConnectSQLServerDemo04 {
		...
		try {
			...
			try {
				if(!objConnection.isClosed()){					
					...
					objCmst = objConnection.prepareCall("{call SP_THEMDULIEULOCATION(?,?)}");
					
					//3.Thiết lập giá trị cho tham số truyền vào
					//objCmst.setInt(1, 5);
					objCmst.setString("LocationName", "HỒ CHÍ MINH CITY VIETNAM");
					objCmst.setBoolean("IsDeleted", true);
					
					//4. Ủy quyền thực thi và nhận về Resultset
					objCmst.execute();
					
					//5.
					objCmst = objConnection.prepareCall("{call SP_HIENTHIDULIEULOCATION()}");
					objRs =  objCmst.executeQuery();
					
					//6. Kiểm tra kết quả
					if(objRs != null){
						while (objRs.next()) {
							System.out.println("- LocationId: "+ objRs.getString("LocationId"));
							System.out.println("- LocationName: "+ objRs.getString("LocationName"));
							System.out.println("- IsDeleted: "+ objRs.getBoolean("IsDeleted"));
							System.out.println();
						}
					}
				}
