package Teacher.ADVANCED_TOPICs.SOCKET_PROGRAMMING.MODULES02;
...
public class ServerApps {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			while (true) {
				new Processing(objListener.accept(), iClientNumber++).start();
			}
		} finally {
			objListener.close();
		}
	...
	private static class Processing extends Thread {
		...
		public Processing(Socket objSocket, int iClientNumber) {
			...
			System.out.println(String.format("- HotName: %s", objSocket.getInetAddress().getHostName()));
			System.out.println();
		}
		...
		public void run() {
			try {
				...
				BufferedReader objBr = new BufferedReader(new InputStreamReader(m_objSocket.getInputStream()));
				PrintWriter objPw = new PrintWriter(m_objSocket.getOutputStream(), true);

				// 2. Gửi 1 thông điệp "welcome" đến client.
				objPw.println(String.format("[%s]: Đồng ý kết nối với Client!!!", new Date().toLocaleString()));
				...
				while (true) {
					...
					objPw.println(String.format("[%s]: Em vừa gõ cái này đúng chưa? - %s", new Date().toLocaleString(),
							sData));
				}
			} catch (IOException e) {
