package Teacher.ADVANCED_TOPICs.SOCKET_PROGRAMMING.MODULES01;
...
public class ServerApps {
	...
	/**
	 * 1 TCP Server sẽ chạy trên cổng 9090. Khi có 1 Client kết nối đến, nó sẽ gửi
	 * về Client thông tin về thời gian hiện thời(date and time). Sau đó đóng kết
	 * nối với client.
	 */
		...
		try {
			...
			while (true) {
				...
				try {
					...
					PrintWriter objPW = new PrintWriter(objSocket.getOutputStream(), true);
					objPW.println(String.format("[%s]: Đồng ý kết nối cho Client !!!", new Date().toLocaleString()));
				} finally {
