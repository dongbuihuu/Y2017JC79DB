		...
		public void run() {
			try {
				...
				m_objBr = new BufferedReader(
							new InputStreamReader(m_objSocket.getInputStream()));
				...
				while (true) {
					...
					synchronized (lisNames) {
						...
						if (!lisNames.contains(sName)) {
							lisNames.add(sName);
							System.out.println(String.format("- Coms Name: %s", 
									sName)); 
							break;
						}
					}
				...
				while (true) {
					...
					if (sData == null)
						...
						System.out.println(String.format("[%s]: Nội dung Client gửi về: %s", 
